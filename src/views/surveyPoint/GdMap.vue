  ...
  <div id="GDMap" class="amap-wrapper"  v-loading="loading">
    <div class="input-card" style="width:290px;" v-if="isShowPick">
      <el-radio-group v-model="radioValue" @change="onModeChange" >
        <el-radio label="dragMarker">拖拽Marker模式</el-radio>
...
export default {
  ...
  methods: {
    ...
    init () {
      this.loading = true
      const center = this.showLngLat ? gcoord.transform(
        this.showLngLat.split(','), // 经纬度坐标
        gcoord.WGS84, // 当前坐标系
        gcoord.GCJ02 // 目标坐标系
      ) : []
      // 加载地图
      ...
      //   if(this.positionPicker.started){//如果
    ...
    codeToLocaltion (location) { // 输入84坐标系 转为高德坐标系 再反编码出地址
      ...
      if (location.indexOf(',') > -1 && location.indexOf(',') === location.lastIndexOf(',')) {
        ...
        if (!lngBoolean || !latBoolean) {
          ...
          this.geocoder.getAddress(tempResult, (status, result) => {
...
116.355183~121.927472 30.76028~35.127197

1.经度正则为：/^1[1-2][0-9]\.\d{6,12}$/

2、纬度正则为：/^((3[0-6])|(2[8-9]))\.\d{6,12}$/

var longrg = /^(73\.(3[3-9]{1}\d{0,22222})|([4-9]{1}\d{0,22222})) | (7[4-9]{1}\.\d{0,222222}) | ([8-9]{1}\d{1}\.\d{0,222222}) | (1[0-2]{1}\d{1}\.\d{0,222222}) | (13[0-4]{1}\.\d{0,222222}) | (135\.0[0-5]{0,1})$/
var latreg = /^(3\.5[1-9]{1}\d{0,22222}) | (3\.[6-9]{1}\d{0,222222}) | ([4-9]{1}\.\d{0,222222}) | ([1-4]{1}\d{1}\.\d{0,222222}) | (5[0-2]{1}\.\d{0,222222}) | (53\.[0-2]{1}\d{0,222222}) |(53\.3[0-3]{0,1})$/
